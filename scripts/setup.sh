...
#   --demo=NAME                   Deploy specific demo (movies, books, autozone, autodining, autocrm, automail,
#                                 autodelivery, autolodge, autoconnect, autowork, autocalendar, autolist, autodrive, all)
#   --enable_dynamic_html=BOOL    Enable dynamic HTML (true/false, default: false)
#   --enable_data_generation=BOOL Enable mock data generation (true/false, default: false)
#   --enable_db_mode=BOOL         Enable database mode (true/false, default: false)
#   --seed_value=VAL              Optional seed for generated data
#   -y, --yes                     Force prune/remove containers, images, and volumes
#
# Examples:
#   ./setup.sh --demo=autowork --enable_data_generation=true --web_port=8002
#   ./setup.sh --demo=all -y
#------------------------------------------------------------
...
if [ "$FORCE_DELETE" = true ]; then
  echo "[INFO] Force cleanup enabled. Removing all containers, images, and volumes..."
...
deploy_project() {
  ...
  ENABLE_DATA_GENERATION="$ENABLE_DATA_GENERATION" NEXT_PUBLIC_ENABLE_DATA_GENERATION="$ENABLE_DATA_GENERATION" \
  ENABLE_DB_MODE="$ENABLE_DB_MODE" NEXT_PUBLIC_ENABLE_DB_MODE="$ENABLE_DB_MODE" \
  DATA_SEED_VALUE="$SEED_VALUE" NEXT_PUBLIC_DATA_SEED_VALUE="$SEED_VALUE" \
  API_URL="http://app:$WEBS_PORT" NEXT_PUBLIC_API_URL="http://localhost:$WEBS_PORT" \
  docker compose -p "$proj" up -d --build
...
deploy_webs_server() {
  ...
  while [ $attempt -le $max_attempts ]; do
    ...
    echo "   Attempt $attempt/$max_attempts - not ready, waiting 2s..."
    sleep 2
    attempt=$((attempt + 1))
  done
  ...
  popd >/dev/null
  echo "✅ $name running on localhost:$WEBS_PORT"
  echo
}

#------------------------------------------------------------
# 8. Execute based on demo
#------------------------------------------------------------
case "$WEB_DEMO" in
  ...
  autozone)      deploy_webs_server; deploy_project "web_3_autozone" "$WEB_PORT" "" "autozone_${WEB_PORT}" ;;
  autodining)    deploy_webs_server; deploy_project "web_4_autodining" "$WEB_PORT" "" "autodining_${WEB_PORT}" ;;
  autocrm)       deploy_webs_server; deploy_project "web_5_autocrm" "$WEB_PORT" "" "autocrm_${WEB_PORT}" ;;
  automail)      deploy_webs_server; deploy_project "web_6_automail" "$WEB_PORT" "" "automail_${WEB_PORT}" ;;
  autodelivery)  deploy_webs_server; deploy_project "web_7_autodelivery" "$WEB_PORT" "" "autodelivery_${WEB_PORT}" ;;
  autolodge)     deploy_webs_server; deploy_project "web_8_autolodge" "$WEB_PORT" "" "autolodge_${WEB_PORT}" ;;
  autoconnect)   deploy_webs_server; deploy_project "web_9_autoconnect" "$WEB_PORT" "" "autoconnect_${WEB_PORT}" ;;
  autowork)      deploy_webs_server; deploy_project "web_10_autowork" "$WEB_PORT" "" "autowork_${WEB_PORT}" ;;
  autocalendar)  deploy_webs_server; deploy_project "web_11_autocalendar" "$WEB_PORT" "" "autocalendar_${WEB_PORT}" ;;
  ...
  *)
    echo "❌ Invalid demo option: $WEB_DEMO"
    echo "   Valid: movies, books, autozone, autodining, autocrm, automail, autodelivery, autolodge, autoconnect, autowork, autocalendar, autolist, autodrive, all"
    exit 1 ;;
esac
