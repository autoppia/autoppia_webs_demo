...
#   --demo=NAME                   Deploy specific demo: movies, autocinema, books, autobooks, autozone, autodining, autocrm, automail, autodelivery, autolodge, autoconnect, autowork, autocalendar, autolist, autodrive, autohealth, autochess, or all (default: all)
...
WEBS_PROJECTS="web_1_autocinema web_2_autobooks web_3_autozone web_4_autodining web_5_autocrm web_6_automail web_7_autodelivery web_8_autolodge web_9_autoconnect web_10_autowork web_11_autocalendar web_12_autolist web_13_autodrive web_14_autohealth web_17_autochess"
...
setup_docker() {
  ...
  if [ "$FAST_MODE" = true ]; then
    ...
    if [ "$CLEAN_ALL" = true ]; then
      ...
      local project_prefixes="webs_server movies_ books_ autozone_ autodining_ autocrm_ automail_ autodelivery_ autolodge_ autoconnect_ autowork_ autocalendar_ autolist_ autodrive_ autohealth_ autochess_"
...
case "$WEB_DEMO" in
  ...
  autochess)
    deploy_webs_server
    deploy_project "web_17_autochess" "$WEB_PORT" "" "autochess_${WEB_PORT}"
    ;;
  all)
    deploy_webs_server
    ...
    run_with_limit deploy_project "web_17_autochess" "$((WEB_PORT + 16))" "" "autochess_$((WEB_PORT + 16))"
    failed=0
    ...
    ;;
  *)
    echo "❌ Invalid demo: $WEB_DEMO. Use one of: movies, autocinema, books, autobooks, autozone, autodining, autocrm, automail, autodelivery, autolodge, autoconnect, autowork, autocalendar, autolist, autodrive, autohealth, autochess, or all."
    exit 1
    ;;
esac
