...
#   --enable_dynamic_html=BOOL    Enable dynamic HTML (legacy flag)
...
echo "    Seed structure enabled: →  $DYNAMIC_HTML_STRUCTURE"
echo
...
deploy_project() {
  ...
  echo "📂 Deploying $name (HTTP→$webp, DB→$pgp, Dynamic HTML→$ENABLE_DYNAMIC_HTML, Seed Structure→$DYNAMIC_HTML_STRUCTURE)..."
  pushd "$dir" > /dev/null
  ...
  popd > /dev/null
  echo "✅ $name is running on port $webp (Dynamic HTML: $ENABLE_DYNAMIC_HTML, Seed Structure: $DYNAMIC_HTML_STRUCTURE)"
  echo
}
