...
for ARG in "$@"; do
  case $ARG in
    ...
    --enable_dynamic_html_structure=*) ENABLE_DYNAMIC_HTML_STRUCTURE="${ARG#*=}" ;;
...
deploy_project() {
  ...
  pushd "$dir" > /dev/null
    ...
    # Pass dynamic flags to all webs
  ...
  popd > /dev/null
  echo "✅ $name is running on port $webp (Dynamic HTML: $ENABLE_DYNAMIC_HTML, Structure: $ENABLE_DYNAMIC_HTML_STRUCTURE)"
  echo
}
