...
#   --enable_dynamic_html=BOOL    Enable dynamic HTML (true/false, default: true)
#   --enable_data_generation=BOOL Enable data generation (true/false, default: false)
...
deploy_project() {
  ...
  echo "📂 Deploying $name (HTTP→$webp, DB→$pgp, Dynamic HTML→$ENABLE_DYNAMIC_HTML, Data Gen→$ENABLE_DATA_GENERATION)..."
  pushd "$dir" > /dev/null
  ...
  popd > /dev/null
  echo "✅ $name is running on port $webp (Dynamic HTML: $ENABLE_DYNAMIC_HTML, Data Gen: $ENABLE_DATA_GENERATION)"
  echo
}
