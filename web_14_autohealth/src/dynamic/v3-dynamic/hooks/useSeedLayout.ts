...
export function useSeedLayout() {
  ...
  const layoutConfig = useMemo(
    () => getEffectiveLayoutConfig(layoutSeed),
    [layoutSeed]
  );
  const layoutClasses = useMemo(
    () => getLayoutClasses(layoutConfig),
    [layoutConfig]
  );
  ...
  const getLayoutClassTokens = useCallback(
  ...
  const getLayoutInfo = useCallback(() => {
    return {
      ...
      cssVariables,
      isDynamicEnabled,
      layoutType: layoutVariant.layoutType,
      buttonLayout: layoutVariant.buttonLayout,
      labelStyle: layoutVariant.labelStyle,
      spacing: layoutVariant.spacing,
      elementOrder: layoutVariant.elementOrder,
    };
  }, [seed, layoutVariant, cssVariables, isDynamicEnabled]);
