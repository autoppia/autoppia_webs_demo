...
export function DynamicPopup({ variant, onClose }: DynamicPopupProps) {
  ...
  const content = (
    ...
    >
      <div ref={dialogRef} tabIndex={-1} className={`relative rounded-xl border border-border bg-card text-card-foreground shadow-lg px-6 py-6 sm:px-8 sm:py-8 ${getPlacementClasses(variant.placement)}`} style={placementStyle} data-popup-id={variant.popupId}>
