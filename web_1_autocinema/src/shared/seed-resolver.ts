/**
 * Simplified seed utilities for autocinema.
 * All variants share the same base seed from the URL.
 */
...
export async function resolveSeeds(baseSeed: number): Promise<ResolvedSeeds> {
  ...
  const seeds = buildSeeds(safeSeed);
  resolvedCache.set(safeSeed, seeds);
  return seeds;
}
