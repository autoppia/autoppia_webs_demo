/**
 * Seed resolution client (autocinema)
 * Always resolves seeds through the backend /seeds/resolve endpoint.
 */

const BOOL_TRUE = ["true", "1", "yes", "y"];

const boolFromEnv = (value?: string | null): boolean => {
...
function getFallbackResolved(seed: number): ResolvedSeeds {
  ...
  // v1 and v3 mirror the URL/base seed; only v2 can differ
 ...
 * No local formulas are used — backend is the single source of truth.
 */
  ...
  const promise = (async () => {
    try {
      ...
      const resolved: ResolvedSeeds = {
        ...
        v1: safeSeed, // v1 mirrors base/url seed
        v2: data.v2 ?? null,
        v3: safeSeed, // v3 mirrors base/url seed
      };
...
/**
 * Synchronous snapshot of the last resolved seeds (or base-only placeholder).
 */
