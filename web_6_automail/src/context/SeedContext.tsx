...
function SeedInitializer({
  ...
  useEffect(() => {
    ...
    if (enableDynamic) {
      console.log("[SeedContext:web6] enable_dynamic from URL (user override):", enableDynamic);
    } else {
      console.log("[SeedContext:web6] No enable_dynamic in URL, using env vars as default");
    }
...
export const SeedProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (urlSeed !== null && !isNaN(urlSeed)) {
      ...
      console.log(`[SeedContext:web6] Using seed from URL: ${urlSeed}`);
    } else {
      // No seed in URL, try localStorage
      try {
        const savedSeed = localStorage.getItem("automail_seed_base");
        if (savedSeed) {
          const parsedSeed = clampBaseSeed(Number.parseInt(savedSeed, 10));
          setSeedState(parsedSeed);
          console.log(`[SeedContext:web6] Using seed from localStorage: ${parsedSeed}`);
        }
      } catch (error) {
  ...
  const handleSeedFromUrl = useCallback((urlSeed: number | null) => {
    if (urlSeed !== null) {
      console.log(`[SeedContext:web6] Seed changed in URL: ${urlSeed}`);
      setSeedState(urlSeed);
      setUrlSeedProcessed(true);
      ...
      console.log(`[SeedContext:web6] Seed removed from URL, using default: ${DEFAULT_SEED}`);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("automail_seed_base", seed.toString());
    } catch (error) {
