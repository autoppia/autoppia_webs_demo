...
// ============================================================================
// TEST V2: Dataset variation (optional integration test)
// ============================================================================

function testV2DatasetVariation() {
  console.log('\n🗃️  TEST V2: Variación de dataset (opcional)');
  ...
  // Default strict: fail if backend is down OR if dataset does not vary across seeds.
  ...
  // Use curl synchronously to avoid async changes in this script.
  let execFileSync;
  try {
    ({ execFileSync } = require('child_process'));
  } catch {
    console.log('   ⚠️  Saltado: no se pudo cargar child_process');
    return results;
  }
  ...
  if (unique.size >= 2) {
    console.log(`   ✅ Variación detectada: ${unique.size}/${seedsToTest.length} resultados distintos (por top-10 ids/titles)`);
    results.passed++;
  } else {
    const msg = `No se detectó variación en V2 (todos los seeds devolvieron el mismo top-10). Esto puede ser normal si el backend no usa seed_value o si el dataset/algoritmo colisiona.`;
    if (strict) {
      console.log(`   ❌ ${msg}`);
      results.failed++;
      results.errors.push(msg);
...
function runAllTests() {
  ...
  results.push(testV2DatasetVariation()); // OPTIONAL
