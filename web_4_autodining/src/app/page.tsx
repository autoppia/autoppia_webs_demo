...
import { CalendarIcon, ClockIcon, UserIcon, ChevronLeft, ChevronRight, Search, SearchX, MapPin, Star } from "lucide-react";
        ...
        {dyn.v1.addWrapDecoy("home-search-section", (
          ...
          >
            <div className="flex flex-wrap gap-3 items-center glass rounded-2xl p-3 shadow-2xl shadow-black/30 max-w-4xl w-full">
              ...
              <div className="w-full flex items-center gap-2 overflow-x-auto px-1 py-1">
                <button
                  type="button"
                  className={cn(
                ...
                {tagOptions.map((tag) => (
                  <button
                    ...
                    onClick={() => {
                      setSelectedTag(tag);
                      logEvent(EVENT_TYPES.TAG_FILTER_SELECTED, { tag, action: "select", search });
                    }}
                  >
                    {tag}
                  </button>
