...
function testV2DatasetVariation() {
  ...
  function buildUrl(seed, entityType) {
  ...
  // AutoCRM uses multiple entities; verify at least clients vary across seeds.
  ...
  try {
    seedsToTest.forEach((seed) => {
      const url = buildUrl(seed, entityType);
      ...
      console.log(`   📦 ${entityType} seed=${seed}: items=${data.length}`);
    });
  } catch (err) {
  ...
  if (unique.size >= 2) {
    ...
    const msg = `No se detectó variación en V2 para entity_type=${entityType} (todos los seeds devolvieron el mismo top-10).`;
    if (strict) {
      console.log(`   ❌ ${msg}`);
      results.failed++;
      results.errors.push(msg);
