...
function testEventCoverage() {
  ...
  if (!eventTypesMatch) {
    console.log('\n❌ Could not extract EVENT_TYPES from file');
    results.failed++;
...
function generateReport(result) {
  ...
  console.log('\n' + '='.repeat(60));
  if (result.failed === 0) {
    console.log('✅ EVENT COVERAGE: 100% - VALIDATION SUCCESSFUL');
    console.log('   All events are in use.');
  } else {
    console.log('❌ EVENT COVERAGE: REQUIRES ATTENTION');
