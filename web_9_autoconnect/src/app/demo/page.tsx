...
export default function DemoPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <h1 className="text-3xl font-bold mb-8">Dynamic Layout Demo</h1>
      <p className="text-gray-600 mb-8">
        This page demonstrates the different layout variations based on seed values. 
        Each seed (1-10) creates a unique layout arrangement to confuse scraper agents.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {seeds.map((seed) => (
          ...
          >
            <h2 className="text-xl font-semibold mb-2">Seed {seed}</h2>
            <p className="text-gray-600 text-sm mb-4">
              Click to view the layout with seed={seed}
            </p>
            <div className="text-xs text-gray-500">
              <div>• Dynamic element positioning</div>
              <div>• Shuffled navigation order</div>
              <div>• Reordered content feed</div>
              <div>• Varied sidebar placement</div>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="mt-8 p-6 bg-green-50 rounded-lg">
        <h3 className="text-lg font-semibold mb-2">Anti-Scraping Features:</h3>
        <ul className="text-sm text-gray-700 space-y-1">
