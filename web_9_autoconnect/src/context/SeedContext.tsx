...
declare global {
  interface Window {
    __autoconnectV2Seed?: number | null;
  }
  interface WindowEventMap {
    "autoconnect:v2SeedChange": CustomEvent<{ seed: number | null }>;
  }
}
...
function SeedInitializer({
  ...
  useEffect(() => {
    ...
    if (enableDynamic) {
      console.log("[SeedContext:web9] enable_dynamic from URL (user override):", enableDynamic);
    } else {
      console.log("[SeedContext:web9] No enable_dynamic in URL, using env vars as default");
    }
...
export const SeedProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (urlSeed !== null && !isNaN(urlSeed)) {
      ...
      console.log(`[SeedContext:web9] Using seed from URL: ${urlSeed}`);
    } else {
      // No seed in URL, try localStorage
      try {
        const savedSeed = localStorage.getItem("autoconnect_seed_base");
        if (savedSeed) {
          const parsedSeed = clampBaseSeed(Number.parseInt(savedSeed, 10));
          setSeedState(parsedSeed);
          console.log(`[SeedContext:web9] Using seed from localStorage: ${parsedSeed}`);
        }
      } catch (error) {
  ...
  const handleSeedFromUrl = useCallback((urlSeed: number | null) => {
    if (urlSeed !== null) {
      console.log(`[SeedContext:web9] Seed changed in URL: ${urlSeed}`);
      setSeedState(urlSeed);
      setUrlSeedProcessed(true);
      ...
      console.log(`[SeedContext:web9] Seed removed from URL, using default: ${DEFAULT_SEED}`);
  ...
  useEffect(() => {
    ...
    resolveSeeds(seed).then((resolved) => {
      ...
      if (!cancelled) {
        console.warn("[SeedContext:web9] Failed to resolve seeds from API, using local:", error);
      }
    });
    
    try {
      localStorage.setItem("autoconnect_seed_base", seed.toString());
    } catch (error) {
  ...
  useEffect(() => {
    ...
    (window as any).__autoconnectV2Seed = v2Seed ?? null;
    window.dispatchEvent(new CustomEvent("autoconnect:v2SeedChange", { detail: { seed: v2Seed ?? null } }));
    console.log("[SeedContext:web9] v2-seed synced to window:", v2Seed);
