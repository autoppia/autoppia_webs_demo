...
import { Search as SearchIcon, X, TrendingUp, Calendar, Book, Star, ArrowUpDown } from "lucide-react";
...
function SearchContent() {
  ...
  const handleGenreChange = (value: string) => {
    ...
    logEvent(EVENT_TYPES.FILTER_BOOK, {
  ...
  const stats = useMemo(() => {
    ...
    const totalRating = filteredBooks.reduce((acc, book) => acc + book.rating, 0);
    const totalPages = filteredBooks.reduce((acc, book) => acc + book.duration, 0);
    const minYear = Math.min(...filteredBooks.map((b) => b.year));
    const maxYear = Math.max(...filteredBooks.map((b) => b.year));

    return {
      avgRating: (totalRating / filteredBooks.length).toFixed(1),
      avgPages: Math.round(totalPages / filteredBooks.length),
      ...
      <div className="w-full bg-gradient-to-br from-[#0a0d14] via-[#141926] to-[#0F172A] relative" id={dyn.v3.getVariant("search-page", ID_VARIANTS_MAP, "search-page")}>
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-white/70">
            Find your perfect book with our advanced search engine
          </p>
        </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            >
              ...
              <option value="duration-desc" className="bg-neutral-900 text-white">Pages: Most First</option>
              <option value="duration-asc" className="bg-neutral-900 text-white">Pages: Least First</option>
            </select>
          </div>
        </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
                <div className="flex items-center gap-2 text-white/60 mb-1">
                  ...
                  <span className="text-xs font-semibold uppercase">Avg Pages</span>
                </div>
                <div className="text-2xl font-bold text-white">{stats.avgPages}</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
                <div className="flex items-center gap-2 text-white/60 mb-1">
