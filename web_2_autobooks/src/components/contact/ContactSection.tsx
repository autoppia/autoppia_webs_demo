...
export function ContactSection() {
  ...
  return (
    <section
      id="contact"
      className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#0c101a] via-[#05070d] to-[#030508] p-6 text-white shadow-2xl"
    >
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-secondary">Contact</p>
          <h2 className="mt-3 text-3xl font-semibold">Need a curated reading list?</h2>
          <p className="mt-2 text-sm text-white/70">
            Tell us the vibe you are targeting and we will route the request to the appropriate validator. Every message
            generates an event so you can audit submissions easily.
          </p>
          <ul className="mt-4 space-y-2 text-sm text-white/70">
            <li>• Custom AI bookshelf briefs</li>
            <li>• Partner inquiries &amp; devrel</li>
            <li>• Bug reports from miners</li>
          </ul>
          {status === "sent" && (
            <p className="mt-4 rounded-xl border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-sm text-emerald-200">
              Message logged. The validator team will pick it up.
            </p>
          )}
        </div>
