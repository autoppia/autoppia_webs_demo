...
import { Sparkles, TrendingUp, BookOpen, Star, Search as SearchIcon } from "lucide-react";
...
export function HeroSection({
  ...
  const dynamicV3IdsVariants: Record<string, string[]> = {
    ...
    button: ["hero-view-details-btn", "view-details-button", "details-action", "view-book-btn", "details-btn"],
  };
  ...
  const stats = useMemo(() => {
    const totalPages = featuredBooks.reduce((acc, book) => acc + book.duration, 0);
    return {
      avgPages: featuredBooks.length ? Math.round(totalPages / featuredBooks.length) : 320,
      avgRating: featuredBooks.length
        ? (featuredBooks.reduce((acc, book) => acc + book.rating, 0) / featuredBooks.length).toFixed(1)
        : "4.0",
      genres: new Set(featuredBooks.flatMap((book) => book.genres)).size,
    };
  }, [featuredBooks]);

  const topBooks = featuredBooks.slice(0, 3);
          ...
          <div className="grid gap-12 lg:grid-cols-12 lg:gap-12">
            ...
            <div className="lg:col-span-5 xl:col-span-4">
              <div className="flex items-center gap-2 text-sm font-medium text-white/80 mb-6">
                ...
                <span>Curated AI book collections</span>
              </div>
              
              <h1 className="text-5xl font-bold leading-[1.1] tracking-tight lg:text-6xl xl:text-7xl mb-6">
                Discover AI-driven stories, remixed genres, and reading experiments.
              </h1>
              
              <p className="text-lg text-white/70 leading-relaxed mb-8">
                Search hundreds of procedurally generated books loaded directly from our datasets service. No backend, no forms – just literature.
              </p>
              ...
              >
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder="Search authors, titles, or moods"
              ...
              <dl className="grid grid-cols-3 gap-4">
                <div className="group rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm transition-all hover:border-secondary/30 hover:bg-white/10 hover:scale-105">
                  <dt className="text-xs font-semibold uppercase tracking-wider text-white/60 mb-2">Avg pages</dt>
                  <dd className="text-4xl font-bold text-white">{stats.avgPages}p</dd>
                </div>
                <div className="group rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm transition-all hover:border-secondary/30 hover:bg-white/10 hover:scale-105">
            ...
            {/* Right side - Featured books showcase */}
