...
test_seed_behavior() {
    ...
    if [ "$response" = "200" ]; then
        ...
        if [ "$xpath_attrs" -eq 0 ]; then
            echo -e "${GREEN}   ✅ No XPath confusion attributes found (expected when enabled)${NC}"
        else
...
echo "🔍 Checking if web_11_autocalendar is running on port 8010..."
...
echo "  http://localhost:8010/?seed=1   (Default)"
echo "  http://localhost:8010/?seed=5   (Should be same as seed=1)"
echo "  http://localhost:8010/?seed=180 (Should be same as seed=1)"
echo "  http://localhost:8010/?seed=200 (Should be same as seed=1)"
echo "  http://localhost:8010/          (No seed parameter)"
