...
declare global {
  ...
  interface WindowEventMap {
    "autolodge:v2SeedChange": CustomEvent<{ seed: number | null }>;
  }
}
...
function SeedInitializer({
  ...
  useEffect(() => {
    ...
    if (enableDynamic) {
      console.log("[SeedContext:web8] enable_dynamic from URL (user override):", enableDynamic);
    } else {
      console.log("[SeedContext:web8] No enable_dynamic in URL, using env vars as default");
    }
...
export const SeedProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        const savedSeed = localStorage.getItem("autolodge_seed_base");
        if (savedSeed) {
          const parsedSeed = clampBaseSeed(Number.parseInt(savedSeed, 10));
          setSeedState(parsedSeed);
          console.log(`[SeedContext:web8] Using seed from localStorage: ${parsedSeed}`);
        }
      } catch (error) {
  ...
  const handleSeedFromUrl = useCallback((urlSeed: number | null) => {
    if (urlSeed !== null) {
      console.log(`[SeedContext:web8] Seed changed in URL: ${urlSeed}`);
      ...
      console.log(`[SeedContext:web8] Seed removed from URL, using default: ${DEFAULT_SEED}`);
  ...
  useEffect(() => {
    ...
    resolveSeeds(seed).then((resolved) => {
      ...
      if (!cancelled) {
        console.error("[SeedContext:web8] Failed to resolve seeds from API:", error);
        // Keep default values (with nulls for v1/v2/v3) - will retry on next seed change
      }
    });
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem("autolodge_seed_base", seed.toString());
      } catch (error) {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent("autolodge:v2SeedChange", { detail: { seed: v2Seed ?? null } }));
    console.log("[SeedContext:web8] v2-seed synced to window:", v2Seed);
