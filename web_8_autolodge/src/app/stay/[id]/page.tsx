...
function PropertyDetailContent() {
  ...
  const createEventElement = (eventType: string, index: number) => {
    switch (eventType) {
      ...
      case 'reserve':
        return (
          <DynamicWrapper key={`reserve-${index}`} as="div" className="bg-white shadow-md rounded-2xl border flex flex-col p-6">
                ...
                onClick={async () => {
                  ...
                  try {
                    await logEvent(EVENT_TYPES.RESERVE_HOTEL, {
                      ...
                      guests_set: guests,
                      hotel: prop,
                      selected_checkin: checkinDate.toLocaleDateString('en-CA'),
                      selected_checkout: checkoutDate.toLocaleDateString('en-CA'),
      ...
      case 'wishlist':
        return (
          <DynamicWrapper key={`wishlist-${index}`} as="div" className="bg-white rounded-lg border p-6">
            <h3 className="text-lg font-semibold mb-4">Reviews & Actions</h3>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1">
                ...
                <span className="font-medium text-neutral-900">{prop.rating.toFixed(2)}</span>
              </div>
