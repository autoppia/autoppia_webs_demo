...
def login_view(request, variant=None):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # ensure validator_id is never None to avoid DB NOT NULL violations
            login_event = Event.create_login_event(user, web_agent_id, validator_id=request.headers.get("X-Validator-Id", ""))
            login_event.save()
...
def register_view(request, variant=None):
    ...
    if request.method == "POST":
        ...
        if not error:
            ...
            web_agent_id = request.headers.get("X-WebAgent-Id", "0")
            register_event = Event.create_registration_event(user, web_agent_id, validator_id=request.headers.get("X-Validator-Id", ""))
            register_event.save()
            # login(request, user)
            # login_event = Event.create_login_event(user, web_agent_id)
            # login_event.save()
