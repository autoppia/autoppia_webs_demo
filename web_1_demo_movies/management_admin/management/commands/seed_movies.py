...
try:
    ...
    # If movieapp.data_generator isn't importable (e.g., outside Django), ignore
    generate_movies_data = None
    generate_genres_data = None
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # If enabled, use AI-powered data generation instead of static seeds
        if os.environ.get("ENABLE_DATA_GENERATION", "").lower() in ("true", "1", "yes", "on") and generate_movies_data and generate_genres_data:
            logger.info("seed_movies: ENABLE_DATA_GENERATION is true — using AI generator")

            # Optional counts via env vars
            genres_count = int(os.environ.get("SEED_GENRES_COUNT", "16"))
            movies_count = int(os.environ.get("SEED_MOVIES_COUNT", "10"))

            # Generate genres
            genres_result = generate_genres_data(genres_count)
            if not genres_result.success:
                logger.error("seed_movies: generate_genres_data failed: %s", genres_result.error)
                self.stdout.write(self.style.ERROR(f"generate_genres_data failed: {genres_result.error}"))
                return
            logger.info("seed_movies: generated %s genres", genres_result.count)
            self.stdout.write(self.style.SUCCESS(f"Generated and saved {genres_result.count} genres"))

            # Generate movies
            movies_result = generate_movies_data(movies_count)
            if not movies_result.success:
                logger.error("seed_movies: generate_movies_data failed: %s", movies_result.error)
                self.stdout.write(self.style.ERROR(f"generate_movies_data failed: {movies_result.error}"))
                return
            logger.info("seed_movies: generated %s movies", movies_result.count)
            self.stdout.write(self.style.SUCCESS(f"Generated and saved {movies_result.count} movies"))

            # Optionally add comments via existing logic: grab created movies and add comments
            created_movies = Movie.objects.all().order_by('-id')[:movies_result.count]
            # If necessary, add comments (reuse existing comments seeding below)
            # Fall through to comment creation section by building created_movies list

        else:
            logger.info("seed_movies: Using static seed data (AI generation disabled)")
        ...
        logger.info("seed_movies: completed seeding and comments")
