...
export default function PlayerDetailPage() {
  ...
  const playerGames = useMemo(
    () => allGames.filter((g) => g.whitePlayer.id === playerId || g.blackPlayer.id === playerId).slice(0, 30),
    [allGames, playerId],
  );
  ...
  const stats = useMemo(() => {
    ...
    let whiteGames = 0, whiteWins = 0, whiteDraws = 0;
    let blackGames = 0, blackWins = 0, blackDraws = 0;
    ...
    for (const g of playerGames) {
      ...
      if (isWhite) {
        whiteGames++;
        if (g.result === "1-0") { whiteWins++; os.wins++; }
        else if (g.result === "1/2-1/2") { whiteDraws++; os.draws++; }
        else { os.losses++; }
      } else {
        blackGames++;
        if (g.result === "0-1") { blackWins++; os.wins++; }
        else if (g.result === "1/2-1/2") { blackDraws++; os.draws++; }
        else { os.losses++; }
      }
    }

    const avgOpponentRating = Math.round(totalOpponentRating / playerGames.length);
    ...
    return {
      whiteGames, whiteWins, whiteDraws, whiteLosses: whiteGames - whiteWins - whiteDraws,
      blackGames, blackWins, blackDraws, blackLosses: blackGames - blackWins - blackDraws,
      avgOpponentRating,
      topOpenings,
    };
    ...
    <div className="py-6">
      ...
      <DynamicWrapper>
        <nav className="flex items-center gap-2 text-sm mb-6">
          ...
          <button onClick={() => router.push("/players")} className="text-zinc-500 hover:text-amber-400 transition-colors">Players</button>
          <span className="text-zinc-600">/</span>
      ...
      <DynamicWrapper>
        <div className="bg-gradient-to-r from-[#1a1412] to-[#1c1917] border border-stone-800/80 rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-6">
            <RatingCard label="Classical" value={player.rating} icon={<Trophy className="h-4 w-4" />} highlight />
            <RatingCard label="Rapid" value={player.rapidRating} icon={<TrendingUp className="h-4 w-4" />} />
            <RatingCard label="Blitz" value={player.blitzRating} icon={<Swords className="h-4 w-4" />} />
            <RatingCard label="Peak Rating" value={player.bestRating} />
            <RatingCard label="Avg Opponent" value={stats?.avgOpponentRating || 0} />
          </div>
        </div>
      </DynamicWrapper>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <DynamicWrapper>
            <div className="bg-[#1c1917] border border-stone-800/80 rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatBlock label="Games Played" value={String(player.gamesPlayed)} />
                <StatBlock label="Win Rate" value={formatPercentage(player.wins, player.gamesPlayed)} color="text-green-400" />
                <StatBlock label="Draw Rate" value={formatPercentage(player.draws, player.gamesPlayed)} />
                <StatBlock label="Loss Rate" value={formatPercentage(player.losses, player.gamesPlayed)} color="text-red-400" />
              </div>

              {/* W/D/L Bar */}
              <div className="mt-4">
                <div className="flex h-3 rounded-full overflow-hidden">
                  <div className="bg-green-500" style={{ width: `${(player.wins / player.gamesPlayed) * 100}%` }} />
                  <div className="bg-zinc-500" style={{ width: `${(player.draws / player.gamesPlayed) * 100}%` }} />
                  <div className="bg-red-500" style={{ width: `${(player.losses / player.gamesPlayed) * 100}%` }} />
                </div>
          ...
          <DynamicWrapper>
            <div className="bg-[#1c1917] border border-stone-800/80 rounded-xl p-5">
              <h2 className="text-lg font-bold text-white mb-4">Rating History</h2>
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={player.ratingHistory}>
                  ...
                  />
                  <YAxis stroke="#666" tick={{ fill: "#888", fontSize: 11 }} domain={["dataMin - 50", "dataMax + 50"]} />
                  <Tooltip
                  ...
                  />
                  <Area type="monotone" dataKey="rating" stroke="#f59e0b" strokeWidth={2} fill="url(#ratingGrad)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </DynamicWrapper>
              ...
              <Table>
                ...
                <TableBody>
                  {playerGames.map((g) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <span className={`inline-block w-4 h-4 rounded-sm border ${isWhite ? "bg-white border-zinc-300" : "bg-zinc-800 border-zinc-600"}`} />
                        </TableCell>
                        <TableCell className="text-white text-sm">
                          {opponent.name}
                          <span className="text-zinc-500 ml-1">({opponent.rating})</span>
                        </TableCell>
                        <TableCell className={`text-center font-semibold text-sm ${resultColor}`}>{resultForPlayer}</TableCell>
                        <TableCell className="text-zinc-400 text-sm hidden md:table-cell truncate max-w-[180px]">{g.opening}</TableCell>
                  ...
                  {playerGames.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center text-zinc-500 py-8">No games found</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
        ...
        <div className="space-y-6">
          ...
          {stats && (
            <DynamicWrapper>
              <div className="bg-[#1c1917] border border-stone-800/80 rounded-xl p-5">
                <h3 className="text-sm font-semibold text-white mb-3">Results by Color</h3>
          ...
          {stats && stats.topOpenings.length > 0 && (
            <DynamicWrapper>
              <div className="bg-[#1c1917] border border-stone-800/80 rounded-xl p-5">
                <h3 className="text-sm font-semibold text-white mb-3">Top Openings</h3>
                <div className="space-y-2.5">
                  {stats.topOpenings.map((o) => (
                    <div key={o.name} className="text-sm">
                      <div className="flex justify-between mb-1">
                        <span className="text-zinc-300 truncate mr-2">{o.name}</span>
                        <span className="text-zinc-500 flex-shrink-0">{o.games}g</span>
                      </div>
                      <div className="flex h-1.5 rounded-full overflow-hidden bg-zinc-800">
                        <div className="bg-green-500" style={{ width: `${(o.wins / o.games) * 100}%` }} />
                        <div className="bg-zinc-500" style={{ width: `${(o.draws / o.games) * 100}%` }} />
                        <div className="bg-red-500" style={{ width: `${(o.losses / o.games) * 100}%` }} />
                      </div>
                      <div className="flex justify-between text-xs mt-0.5 text-zinc-500">
                        <span>+{o.wins} ={o.draws} -{o.losses}</span>
          ...
          <DynamicWrapper>
            <div className="bg-[#1c1917] border border-stone-800/80 rounded-xl p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <FactRow label="Peak Rating" value={formatRating(player.bestRating)} />
                <FactRow label="Rating Change" value={
                  player.ratingHistory.length >= 2
                    ? (() => {
                      const diff = player.ratingHistory[player.ratingHistory.length - 1].rating - player.ratingHistory[player.ratingHistory.length - 2].rating;
                      return diff >= 0 ? `+${diff}` : String(diff);
                    })()
                    : "-"
                } />
                <FactRow label="Active Since" value={formatDate(player.joinDate)} />
              </div>
            </div>
          </DynamicWrapper>
        </div>
...
function RatingCard({ label, value, icon, highlight }: { label: string; value: number; icon?: React.ReactNode; highlight?: boolean }) {
  return (
    <div className={`rounded-lg p-3 text-center ${highlight ? "bg-amber-900/30 border border-amber-700/30" : "bg-white/5 border border-white/5"}`}>
      <div className="flex items-center justify-center gap-1.5 mb-1">
        {icon && <span className={highlight ? "text-amber-400" : "text-zinc-500"}>{icon}</span>}
        <span className="text-xs text-zinc-400">{label}</span>
      </div>
      <div className={`text-lg font-bold ${highlight ? "text-amber-400" : "text-white"}`}>{formatRating(value)}</div>
    </div>
  );
}
...
function ColorStatRow({ color, games, wins, draws, losses }: { color: string; games: number; wins: number; draws: number; losses: number }) {
  if (games === 0) return null;
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <span className={`inline-block w-3 h-3 rounded-sm border ${color === "White" ? "bg-white border-zinc-300" : "bg-zinc-800 border-zinc-600"}`} />
      ...
      <div className="flex h-1.5 rounded-full overflow-hidden bg-zinc-800">
        ...
        <div className="bg-zinc-500" style={{ width: `${(draws / games) * 100}%` }} />
        <div className="bg-red-500" style={{ width: `${(losses / games) * 100}%` }} />
      </div>
      <div className="flex justify-between text-xs mt-0.5 text-zinc-500">
        <span>+{wins} ={draws} -{losses}</span>
        <span className="text-amber-400">{games > 0 ? ((wins / games) * 100).toFixed(0) : 0}%</span>
      </div>
    </div>
  );
}
